...
export class AccountPaginatedListPageComponent {
    ...
    public tryToDelete($event) {        
        ...
        this._accountsService.remove({ account: $event.detail.account, correlationId }).subscribe();
    }
    ...
    public get filteredAccounts() {
        return this.unfilteredAccounts.filter((x) => x.name.toLowerCase().indexOf(this.filterTerm.toLowerCase()) > -1);
    }
}
